...
**用法：** 在场景根节点加 Node，挂载 TestRunner.cs，**默认不运行**（`RunTestsOnReady=false`，防止测试副作用污染战斗全局状态，如修改全局费用）；需要回归时在 Inspector 勾选 `RunTestsOnReady` 后运行。**569 项用例**覆盖：
...
- AI 决策（效用评分端到端：三档分流行为——简单无脑冲用光AP/BFS绕墙/全负效用仍移动；标准攻击门>击杀>伤害/送死剔除（高级）/门与收割豁免/AP=2移动进攻击位连招而AP=1纯接近也走/被墙围死留点/刷怪红格回避与替代（高级）/火力区回避（高级）与逃离火力区（能跑就跑；能推进则不逃）/卡位·压制走位（高级，攻击不到站玩家攻击范围边缘外一格，逼玩家移动）/卡位·贴身堵（高级，逃不掉站玩家身边缩移动范围）/炮灰强制用光AP（全负移动也走不Skip）/炮灰忽略压制位直接深进（高级停环上逼玩家移动对照）/炮灰无视预测被击杀（站被一击反杀格也冲，高级重罚避开）/远程单位不后退不对峙（推进优先）/低血逃跑（高级）满血横移绕路/无更近格横移探索（高级，目标被挡路也不站着不动）/门不算威胁源（带攻击力的门也逼近不后退）/远处7格也主动接近门；稀有度惜命系数——初级/中级炮灰敢穿火线/送死豁免/低血不逃/忽略卡位/无视预测被击杀/永不Skip、高级顶级惜命、系数 0.25/1.2/1.5 与惩罚缩放/逃离奖励（纯函数）；火力区形状感知（十字 4N=8 格/方形 3x3 去中心）；狡诈让位与击杀门豁免/两步前瞻（含 NextTurnValue 纯函数）/预测威胁与预测被击杀惩罚（纯函数）/集火目标加成/弱侧门偷家；行动顺序 RoughPriority：当场击杀先手/高攻>低攻/近门>远门；绕障距离 GetDistanceFrom：通畅/墙挡/队友穿越/玩家挡路）
...
| 玩家火力区 / 刷怪红格 | −6000 / −6000 |
| 预测威胁区（玩家移动后火力，狡诈） | −4000 |
| 预测被击杀格（玩家移动后可一击反杀，狡诈） | −30000 |
| 两步前瞻（狡诈，仅当前打不到时计入） | 门 +15000 / 可击杀 +6000 |
...
> **稀有度惜命系数（2026-08-09）**：牺牲是否值得按单位稀有度分层（`RarityCareFactor`）——**初级/中级=炮灰**（火力区/预测威胁惩罚 ×0.25、**不做送死检查**、**永不逃跑**、**忽略卡位奖励**、**无视预测被击杀惩罚**（不怕死，站"玩家移动后可一击反杀"的格也冲）、**永不 Skip——负效用移动也执行，强制用光所有 AP 往前冲**）；高级=1.2；顶级=1.5（惜命，避免无谓换血，卡位/压制/贴身堵/留点是它们的战术，预测被击杀重罚避让）。2026-08-09 二次调整：规避伤害权重整体再降（中级也当炮灰），解决"保活权重过高使标准/狡诈推进反而不如简单级无脑冲"。刷怪红格惩罚不随稀有度缩放（战术行为，与保命无关）；**逃离火力区奖励 +6000 不随稀有度缩放**——能跑就跑是所有单位的基本生存行为（修复"站在玩家攻击范围内能跑不跑站着挨打"）；"进入火力区的代价"才按惜命系数缩放（炮灰敢进，惜命不进）。火力区按玩家 `AttackShape` 形状生成（菱形/方形/十字/射线等 CellShape 多态，测试断言覆盖十字/方形）。
...
| 刷怪红格 | 无视 | 惩罚 + **主动让位**（正站红格先移开，除非本回合能击杀门） | 同标准 |
...
- **绕路偷家（涌现）**：靠近门按 `PathFinder.GetDistanceFrom` 绕障 BFS 计分（绕墙/绕玩家占据格、AI 队友占据格可穿越防自挡、忽略自身格、不可达回退曼哈顿），火力区/预测区惩罚自然把路径推向防守薄弱侧；狡诈再叠加**两步前瞻**——当前打不到时，选择下回合（Stamina+AttackDistance 内）够得着门/可击杀目标的格
...
- **防来回动**：禁止移回上回合起点（`_lastMoveFrom`）+ 绕远上限（标准 3 / 狡诈 5 格防走丢——狡诈允许更大绕远以支持偷家路径）

...
│   ├── EnemyAI.cs               敌方 AI 执行层（决策与执行分离：收集战场快照 AiBattleData、狡诈行动顺序与集火状态、镜头预告事件 + CameraPanDelay 停顿、节奏控制；决策纯逻辑在 AiTactics——效用评分）
...
└── Utils/
	├── AiAction.cs               AI 决策结果（Attack/Move/Skip + 效用分/理由；决策与执行分离的数据载体）
	├── AiBattleData.cs           AI 战场快照（纯数据：地图/玩家单位/刷怪格/集火目标/上回合起点/等级）
	├── AiTactics.cs             敌人 AI 决策纯逻辑（效用评分系统：目标打分/移动格评分/送死判断/卡位/逃跑/两步前瞻/预测威胁/集火/行动顺序，零 Manager 依赖，可单测）
...
PlayerAction ← 玩家出牌/移动/攻击，点按钮推进
  ↓
EnemyAction ← AI 自动行动（决策与执行分离：AiTactics 效用评分决策 → 镜头预告 → 执行；简单/标准按距门升序、狡诈按粗效用降序先手）
  ↓
RoundEnd（回合结束）
  ├─ RoundEnd 被动效果
...
- AI 决策（效用评分端到端：三档分流行为——简单无脑冲用光AP/BFS绕墙/全负效用仍移动；标准攻击门>击杀>伤害/送死剔除（高级）/门与收割豁免/AP=2移动进攻击位连招而AP=1纯接近也走/被墙围死留点/刷怪红格回避与替代（高级）/火力区回避（高级）与逃离火力区（能跑就跑；能推进则不逃）/卡位·压制走位（高级，攻击不到站玩家攻击范围边缘外一格，逼玩家移动）/卡位·贴身堵（高级，逃不掉站玩家身边缩移动范围）/炮灰强制用光AP（全负移动也走不Skip）/炮灰忽略压制位直接深进（高级停环上逼玩家移动对照）/炮灰无视预测被击杀（站被一击反杀格也冲，高级重罚避开）/群体冲锋（独自面对大炮不敢进 vs 3 只一起敢进，CrowdDilution=1/3）/远程单位不后退不对峙（推进优先）/低血逃跑（高级）满血横移绕路/无更近格横移探索（高级，目标被挡路也不站着不动）/门不算威胁源（带攻击力的门也逼近不后退）/远处7格也主动接近门；稀有度惜命系数——初级/中级炮灰敢穿火线/送死豁免/低血不逃/忽略卡位/无视预测被击杀/永不Skip、高级顶级轻度惜命、系数 0.25/0.8/1.0 与惩罚缩放/逃离奖励（纯函数）；火力区形状感知（十字 4N=8 格/方形 3x3 去中心）；狡诈让位（攻击优先：站红格能击杀门/士兵先杀，攻击无效才让位）/两步前瞻（含 NextTurnValue 纯函数）/预测威胁与预测被击杀惩罚（纯函数）/集火目标加成/弱侧门偷家；行动顺序 RoughPriority：当场击杀先手/高攻>低攻/近门>远门；绕障距离 GetDistanceFrom：通畅/墙挡/队友穿越/玩家挡路）
...
每关可配置敌方 AI 等级（`LevelData.AiLevel`，默认**标准**，存量关卡自动生效，需调弱可在关卡资源上改）。AI 采用**效用评分系统**（2026-08-09 整体重写），**决策与执行分离**：决策纯逻辑在 `Scripts/Utils/AiTactics.cs`（零 Manager 依赖，可单测），执行层 `Scripts/Manager/EnemyAI.cs` 只负责收集战场快照（`AiBattleData`）、狡诈行动顺序、集火状态、镜头预告与节奏控制。

**全局目标优先级：攻击门 > 击杀 > 造成伤害 > 靠近门 > 躲避伤害 > 逃跑**（权重数值保证链式自洽）：

| 项目 | 权重 |
|---|---|
| 攻击门（胜负关键） | +100000 − 400×距离 |
| 一击击杀（CurrentHP ≤ AttackPower） | +50000 + 400×目标攻击力 − 400×距离 |
| 造成伤害 | +3000×伤害量 + 400×目标攻击力 − 400×距离 |
| 靠近门（**相对当前格**：每接近 1 步 +3000，绝对正收益） | 3000/步接近 |
| 逃离火力区（当前站在玩家攻击范围内 → 候选不在；**不随稀有度缩放**） | +6000 |
| 玩家火力区 / 刷怪红格 | −6000 / −6000（火力区按**群体冲锋稀释**：区内/可进入的敌人数 N → ×1/N，人多不怕大炮秒杀） |
| 预测威胁区（玩家移动后火力，狡诈） | −4000 |
| 预测被击杀格（玩家移动后可一击反杀，狡诈） | −30000 |
| 两步前瞻（狡诈，仅当前打不到时计入；**绕障距离**判定——GetDistanceFrom BFS，墙真实挡路） | 门 +15000 / 可击杀 +6000 |
| 卡位·压制走位（**玩家攻击范围边缘外一格**，逼玩家浪费行动点；仅"到达压制位"时给分——环上横移无奖励，防与远程单位对峙；**高级/顶级专属，炮灰忽略**） | +2000 |
| 卡位·贴身堵（逃不掉时站玩家身边 4 格，缩玩家移动范围；堵格豁免火力区惩罚；**高级/顶级专属，炮灰忽略**） | +1500 |
| 集火（狡诈） | +4000 |
| 逃跑（低血量且无有价值行动） | 600/步远离玩家 |

> **门不算威胁源（2026-08-09）**：火力区/预测威胁/送死反杀/低血判定/逃跑距离全部**排除门**——门是攻击目标不是威胁源，冲门挨门打是预期成本。此前带攻击力的门会令前期弱敌人全负效用 → 低血逃跑后退；现在场上只有门时必定逼近。靠近门权重为"相对当前格"的绝对正收益（远处接近 1 步也是 +2000），不会因绝对距离惩罚把单位钉死在原地。

> **稀有度惜命系数（2026-08-09）**：牺牲是否值得按单位稀有度分层（`RarityCareFactor`）——**初级/中级=炮灰**（火力区/预测威胁惩罚 ×0.25、**不做送死检查**、**永不逃跑**、**忽略卡位奖励**、**无视预测被击杀惩罚**（不怕死，站"玩家移动后可一击反杀"的格也冲）、**永不 Skip——负效用移动也执行，强制用光所有 AP 往前冲**）；高级=0.8；顶级=1.0（轻度惜命，卡位/压制/贴身堵/留点是它们的战术，预测被击杀重罚避让）。2026-08-09 二次调整：规避伤害权重整体再降（中级也当炮灰），解决"保活权重过高使标准/狡诈推进反而不如简单级无脑冲"。刷怪红格惩罚不随稀有度缩放（战术行为，与保命无关）；**逃离火力区奖励 +6000 不随稀有度缩放**——能跑就跑是所有单位的基本生存行为（修复"站在玩家攻击范围内能跑不跑站着挨打"）；"进入火力区的代价"才按惜命系数缩放（炮灰敢进，惜命不进）。

> **群体冲锋稀释（2026-08-09）**：大炮/秒杀类单位（超大范围单体）靠"会死"吓退单只敌人——现在看**敌人整体**（`CrowdDilution`）：统计"当前处于或体力内可进入威胁区"的敌方单位数 N，火力区/预测被击杀惩罚 ×1/N——大炮一回合只能处理一个目标，人多一起冲每只实际死亡风险 ≈1/N（N=0/1 不稀释：独自面对大炮该躲就躲）。数据来源 `AiBattleData.EnemyUnits`（存活敌方非门单位，EnemyAI 每决策构建）。

火力区按玩家 `AttackShape` 形状生成（菱形/方形/十字/射线等 CellShape 多态，测试断言覆盖十字/方形）。

**三档行为矩阵：**

| 特性 | 简单 | 标准 | 狡诈 |
|---|---|---|---|
| 攻击选择 | 打分（门>击杀>威胁>距离） | 打分 + **送死剔除**（打不死+会被一击反杀→不打；门/收割豁免） | 标准 + 集火加成 |
| 移动 | BFS 绕障逼近最近玩家，**永不跳过**（用光行动点） | 效用评分，**最高效用 ≤0 → Skip 留行动点**（无收益不做） | 标准 + 弱侧门 + 两步前瞻 + 预测区重罚 |
| 刷怪红格 | 无视 | 惩罚 + **主动让位**（正站红格且**本回合攻击无效**才移开——攻击优先，能杀则杀不能杀再让） | 同标准 |
| 火力区 | 无视 | −6000 | −6000 + 预测威胁 −4000 + 预测被击杀 −30000 |
| 卡位 | 无 | 有 | 有 |
| 逃跑 | 无（无脑冲） | 有（低血量且无有价值行动） | 同标准 |
| 两步前瞻 | 无 | 无 | 有 |
| 集火 / 行动顺序 | 无 / 距门升序 | 无 / 距门升序 | **有** / 粗效用降序（当场可击杀、高攻、近门先手） |
| 进攻目标 | 最近玩家 | 最近门 | **弱侧门**（防守最薄且 BFS 可达——绕后偷家） |

**机制说明：**

- **移动+攻击连招**：仅 AP≥2 时移动评分计入"该格移动后可攻击目标"的价值（AP=1 移动即耗光行动点，攻击价值是假的），攻击价值按**候选格到目标的距离**打分；连招落点同样做送死检查（`IsSuicidalAttack` 支持 `fromPos` 落点反杀覆盖；**门不参与反杀判定**——门是目标不是威胁）
- **绕路偷家（涌现）**：靠近门按 `PathFinder.GetDistanceFrom` 绕障 BFS 计分（绕墙/绕玩家占据格、AI 队友占据格可穿越防自挡、忽略自身格、不可达回退曼哈顿），火力区/预测区惩罚自然把路径推向防守薄弱侧；狡诈再叠加**两步前瞻**——当前打不到时，选择下回合（Stamina+AttackDistance **绕障距离**内，2026-08-09 起不用曼哈顿——曼哈顿会穿墙误判"够得到"）够得着门/可击杀目标的格
- **集火**：狡诈单位攻击未打死的非门玩家 → 成为集火目标（`_focusTarget`，+4000 加成不锁死，目标死亡/回合开始清除）。**集合猛攻**不设专门机制：门权重 100000 使所有单位目标天然汇聚
- **卡位（2026-08-09 按策划语义重做）**：
  - **压制走位**（攻击不到/攻击完后）：站**玩家攻击范围边缘外一格**（距玩家恰为 AttackDistance+1 的环）——下回合玩家想打必须先移动，浪费行动点。压制奖励**条件式给分**：仅在"到达压制位"时（当前不在环上），环上横移无奖励——否则敌人会永远卡在环上与远程单位对峙
  - **贴身堵**（逃不掉：当前在火力区）：站玩家身边 4 格，占据玩家可移动格把玩家移动范围缩到最小；堵格不比原地更危险（都在挨打）→ 豁免火力区惩罚
- **推进优先于逃离（2026-08-09）**：逃离火力区奖励仅在有"更近候选格"时**不生效**（`canApproach`）——否则敌人一走进火力区就被拉回，永远无法推进攻击远程单位（"一直往后退"）。只有无路可进（已贴脸/被围）时才能跑
- **防来回动**：禁止移回上回合起点（`_lastMoveFrom`）+ 绕远上限（标准 5 / 狡诈 8 格防走丢——狡诈允许更大绕远以支持偷家路径）
- **让位优先级（2026-08-09）**：让位（站在刷怪红格主动移开）在**攻击决策之后**——攻击有效（含击杀门/击杀士兵）直接打，"能杀则杀，不能杀再让位"；让位移动含移动+攻击连招价值（AP≥2 移动出红格再打）
- **横移绕路兜底（2026-08-09）**：无更近格（目标被玩家占据格/墙挡路、或已到最近格但攻击距离够不到）时，若存在"距离不变且不在任何惩罚区"的横移格则侧向探索，避免"无威胁满行动点却站着不动"——只在留点前兜底，不倒退、不回头、不进火力区/刷怪格/预测区
